...
- `{{GitHub}}`：用于筛选近期新出现或近期新建的 AI 开源项目。
...
从输入源 `{{GitHub}}` 中，筛选出近期新出现、新建或刚开始活跃的 5 个 AI 开源项目，提取项目标题和对应链接，整理为“AI 开源项目”模块。优先关注新鲜度和 AI 相关性，不要只按 star 选择长期知名项目。
...
- AI 开源项目优先选择近期新项目，不优先选择长期知名的高 star 老项目。
- 坚决排除无关信息、广告及营销类内容。
- 每一条目必须提供原始链接。
